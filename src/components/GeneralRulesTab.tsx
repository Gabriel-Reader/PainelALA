...
export function GeneralRulesTab({ isRep, updateConfig, currentGeneralRuleBlocks }: GeneralRulesTabProps) {
                ...
                <ul className="space-y-4 relative z-10">
                  {block.rules.map((rule, idx) => (
                    <li key={`${block.id}-rule-${idx}-${rule.substring(0, 10)}`} className="flex items-center group/rule">
                      <div className="bg-indigo-100 p-1 rounded border border-indigo-200 mr-4 shrink-0 shadow-sm"><CheckSquare className="h-4 w-4 text-indigo-600" /></div>
